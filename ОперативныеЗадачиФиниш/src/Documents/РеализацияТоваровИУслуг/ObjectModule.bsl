Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Иначе
		...
		|	ЕСТЬNULL(ВТСебестоимостьТоваров.СуммаОстаток, 0) КАК СуммаОстаток,
		|	ЕСТЬNULL(ВТСебестоимостьТоваров.Партия, НЕОПРЕДЕЛЕНО) КАК Партия,
		|	ЕСТЬNULL(ВТСебестоимостьТоваров.КоличествоОстаток, 0) КАК КоличествоОстаток
		|ИЗ
		|	ВТТоварыДокумента КАК ВТТоварыДокумента
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВТСебестоимостьТоваров.Партия.МоментВремени
		|ИТОГИ
		|	МАКСИМУМ(Количество),
		...
		Движения.ЗаданияНаРасчетСебестоимости.Записывать = Истина; //Очищаем задание на расчет себестоимости
