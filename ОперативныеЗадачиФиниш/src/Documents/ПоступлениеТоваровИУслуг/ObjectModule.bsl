Процедура ОбработкаПроведения(Отказ, Режим)
	...
	РезультатЗапроса = МассивРезультатовЗапроса.Получить(1);
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	ВыборкаДетальныеЗаписи.Следующий();
	МетодСписанияСебестоимости = ВыборкаДетальныеЗаписи.МетодСписанияСебестоимости;

	РезультатЗапроса = МассивРезультатовЗапроса.Получить(0);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если МетодСписанияСебестоимости = Перечисления.МетодыСписанияСебестоимости.ФИФО Тогда
		...
		Если Константы.РезервированиеВариант2.Получить() Тогда
			Движение.Свободно = ВыборкаДетальныеЗаписи.Количество;
		КонецЕсли;
		
		Если Константы.РезервированиеВариант2.Получить() = Ложь Тогда
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПоступлениеТовары.Ссылка.Заказ,
	               |	ПоступлениеТовары.Номенклатура,
	               |	ПоступлениеТовары.Количество,
	               |	ПоступлениеТовары.Сумма
	               |ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение.Номенклатура = ВыборкаДетальныеЗаписи.Номенклатура;
		Движение.ЗаказПоставщику = ВыборкаДетальныеЗаписи.Заказ;
