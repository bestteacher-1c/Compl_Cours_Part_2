...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Сизов+
		Если ЗначениеЗаполнено(Объект.ИмяФайла) = Ложь Тогда
	
		Объект.ШрифтЗапрос = Новый Шрифт("Courier New",14,Истина);
		Объект.ЦветФонаЗапроса = WebЦвета.Белый;
	    Элементы.ТекстЗапроса.Шрифт = Объект.ШрифтЗапрос;
		Элементы.ТекстЗапроса.ЦветФона = Объект.ЦветФонаЗапроса;
		
	КонецЕсли;   
	
	Элементы.ЗначениеВФорме.ВыборГруппИЭлементов = ГруппыИЭлементы.ГруппыИЭлементы;
...
Процедура РезультатЗапросаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Объект.ДоступныеТипыДанных.СодержитТип(ТипРасшифровки)
			И РасшифровкаЯчейки <> Неопределено Тогда
		ЗначениеДляПоказа = РасшифровкаЯчейки;
	ИначеЕсли ТипРасшифровки = Тип("Null") Тогда
		ЗначениеДляПоказа = "Null";
	ИначеЕсли ТипРасшифровки = Тип("УникальныйИдентификатор") Тогда
		ЗначениеДляПоказа = "Уникальный идентификатор";
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	#Иначе
		...
		//Сизов +
		ТекстЗапросаВФорме = СокрЛП(Элементы.ТекстЗапроса.ВыделенныйТекст);
		
		НетВыделения = Ложь;
		
		Если ЗначениеЗаполнено(ТекстЗапросаВФорме) = Ложь Тогда
		...
		СтруктураПараметров.Вставить("НетВыделения", НетВыделения);
		
		//Сизов+
		Если ЗначениеЗаполнено(ТекстЗапросаВФорме) Тогда
		...
		КонецЕсли;
			...
			#ИначеЕсли ТонкийКлиент Тогда //Сизов ~
...
Процедура ПослеЗакрытияКонструктораЗапроса(ТекстЗапросаВФорме, СтруктураПараметров) Экспорт
	...
	Если ТекстЗапросаВФорме <> Неопределено Тогда
		...
		//Сизов+
		    Если СтруктураПараметров.НетВыделения = Ложь Тогда
				ТекущийЗапрос.Текст = СтрЗаменить(ТекущийЗапрос.Текст,СтруктураПараметров.ИсходныйТекстЗапроса,ТекстЗапросаВФорме);
			Иначе	
...
//
&НаСервере
Процедура ВыгрузитьЗапросыВРеквизиты(ПараметрыПередачи) Экспорт
	
	ОписаниеЗапросов = ПолучитьИзВременногоХранилища(ПараметрыПередачи.АдресХранилища); // см. Обработки.КонсольЗапросов.ОписаниеЗапросов
...
Процедура ВыгрузитьНастройки(ПараметрыПередачи)
	...
	//Сизов+
	Объект.ЦветФонаЗапроса				= ПолучитьИзВременногоХранилища(ПараметрыПередачи.АдресХранилища).ЦветФонаЗапроса;
	Объект.ЦветШрифт				= ПолучитьИзВременногоХранилища(ПараметрыПередачи.АдресХранилища).ЦветШрифт;
	Объект.ШрифтЗапрос				= ПолучитьИзВременногоХранилища(ПараметрыПередачи.АдресХранилища).ШрифтЗапрос;

	Элементы.ТекстЗапроса.Шрифт	 = Объект.ШрифтЗапрос;
	Элементы.ТекстЗапроса.ЦветФона = Объект.ЦветФонаЗапроса ;
	Элементы.ТекстЗапроса.ЦветТекста = Объект.ЦветШрифт ;
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Элементы.ГруппаТекстЗапроса.Скрыть();
...
Процедура ПомещениеФайловЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КоличествоЗапросов > 0 Тогда
		...
		ИдентификаторТекущегоЗапроса = Объект.Запросы.Получить(КоличествоЗапросов - 1).Идентификатор;
		Модифицированность = Ложь;
...
Процедура ЗаполнитьЗапросыИПараметрыИзОбъектаВнешнейОбработки(ОбъектОбработки)
	...
	Объект.Параметры.Очистить();
	//Сизов+
	Объект.ЦветФонаЗапроса = ОбъектОбработки.ЦветФонаЗапроса;
	Объект.ЦветШрифт = ОбъектОбработки.ЦветШрифт;
...
#Область ПроцедурыСизов

&НаКлиенте
Процедура ВидимостьВыбораФайла(Команда)
	
	Элементы.ГруппаРаботыСФайлом_.Видимость = Не Элементы.ГруппаРаботыСФайлом_.Видимость;

КонецПроцедуры


&НаКлиенте
Процедура УдалитьЗнакиПереносаСтроки(Команда)
	
	НетВыделения = Ложь;
	
	Текст = Элементы.ТекстЗапроса.ВыделенныйТекст;
	
	Если ПустаяСтрока(Текст) = Истина Тогда
	...
	Если НетВыделения = Истина Тогда
...
Процедура БорьбаСNULL(Команда)
	...
	КонецСтроки = ?( (НачалоСтроки = КонецСтроки и НачалоКолонки = КонецКолонки) ИЛИ КонецКолонки > 1
	, КонецСтроки + 1
	, КонецСтроки);
	...
	Для НСтр = НачалоСтроки По КонецСтроки-1 Цикл
		
		Строка = ТекстЗапроса.ПолучитьСтроку(НСтр);
		...
		СкобкаЗакрыта = Ложь;
		БылПервыйНеПробел = Ложь;
		
		Для Позиция = 1 По ДлинаСтроки Цикл
			
			Символ = Сред(Строка, Позиция, 1);
			
			Если БылПервыйНеПробел = Истина Тогда
				
				Если Символ = " " или Символ = "," Тогда
					
					Если СкобкаЗакрыта = Ложь Тогда
						
						СтрокаЗамена = СтрокаЗамена + ", 0)" + Символ;
						
						СкобкаЗакрыта = Истина;
						
					Иначе 
						
						СтрокаЗамена = СтрокаЗамена + Символ;
			...
			Иначе
				...
				ИНаче
					...
					СтрокаЗамена = СтрокаЗамена + "ЕСТЬNULL(" + Символ;
					БылПервыйНеПробел = Истина;
		...
		Если СкобкаЗакрыта = Ложь Тогда
			...
			СтрокаЗамена = СтрокаЗамена + ", 0)";
		
		КонецЕсли;
		
		КонецКолонки = СтрДлина(СтрокаЗамена);
		
		ТекстЗапроса.ЗаменитьСтроку(НСтр, СтрокаЗамена);
		
		Элементы.ТекстЗапроса.УстановитьГраницыВыделения(НСтр, КонецКолонки, НСтр, КонецКолонки);
		
	КонецЦикла;
	
	ОбновитьОтображениеДанных(Элементы.ТекстЗапроса);
...
Процедура ОбновитьПараметры(Команда)
	...
	Если Не ПустаяСтрока(ТекущийЗапрос.Текст) Тогда
		...
		ЗаполнитьПараметрыПриВыполненииЗапроса(ТекущийЗапрос.Текст);
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Текст запроса пустой.'"));
	конецЕсли;
КонецПроцедуры
